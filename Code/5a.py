...
"""
5_baselines.py
Baselines B0–B4 on val or test set.

Run val first to tune B3/B4 thresholds, then test:
  python 5_baselines.py --split val
  python 5_baselines.py --split test

Fixes vs your version:
  ✅ device_map="auto" safe input placement (no .to("cuda") mismatch)
  ✅ optional max_new_tokens override from CLI
  ✅ cleaner generate() / teacher-forcing NLL with correct device
  ✅ keeps your unified triage prompt (B2)
  ✅ keeps retriever gating (B3) + uncertainty gating (B4)
"""
