...
"""
4b_steering_eval.py
Layer × alpha sweep for activation steering (hidden or MLP space) with:

✅ NumPy 2.0 safe RMS estimation (chunked, no copy=False issues)
✅ Regex-based mode evaluation (Answer / Refuse / Conflict) — NOT exact string match
✅ Probe-driven layer selection: --layers top 10 / top10 / explicit / all
✅ Optional ±1 neighbors around selected layers
✅ Saves:
   - sweep summary JSON
   - per-example generations JSONL.GZ (so you can re-score later with an LLM judge
     without re-running the steering)

Alpha normalization:
  If --alpha_unit=rms (recommended):
      h <- h + (alpha * rms[layer]) * v
  where rms[layer] is estimated from activations/{H|M}_P0_train.npy

Usage examples:

# 1) Screening on VAL (fast; n=40)
python 4b_steering_eval.py \
  --split val --template P0 \
  --steer_mode refuse --steer_space hidden \
  --layers top 10 --include_neighbors 1 \
  --alphas 0 0.5 1 2 3 \
  --n 40 --alpha_unit rms \
  --last_token_only 1 --prompt_step_only 0 \
  --log_file steering/steer_refuse_screen.log

# 2) Fine sweep on VAL (after picking candidate layers; n=200)
python 4b_steering_eval.py \
  --split val --template P0 \
  --steer_mode refuse --steer_space hidden \
  --layers 18 19 20 \
  --alphas 0 0.25 0.5 0.75 1 1.25 1.5 1.75 2 \
  --n 200 --alpha_unit rms \
  --last_token_only 1 --prompt_step_only 0

# 3) Mechanistic prefill-only on VAL (causal-ish)
python 4b_steering_eval.py \
  --split val --template P0 \
  --steer_mode refuse --steer_space hidden \
  --layers 19 --alphas 1.25 \
  --n 80 --alpha_unit rms \
  --last_token_only 1 --prompt_step_only 1

# 4) Final frozen run on TEST (full test)
python 4b_steering_eval.py \
  --split test --template P0 \
  --steer_mode refuse --steer_space hidden \
  --layers 19 --alphas 1.25 \
  --n 999999 --alpha_unit rms \
  --last_token_only 1 --prompt_step_only 0
"""
...
from prompts import render_prompt, DECODE_CFG
...
# ──────────────────────────────────────────────────────────────────────────────
# Regex mode classifier (NOT exact string match)
# ──────────────────────────────────────────────────────────────────────────────
MODE_PATTERNS = {
    "refuse": re.compile(
        r"\b("
...
def classify_mode_regex(text: str) -> str:
    t = text.strip()
    # Check conflict first (more specific)
...
def load_or_compute_rms(
    ...
    for start in range(0, n, bs):
        ...
        chunk = np.asarray(X[start:end, :Luse, :], dtype=np.float32)  # NumPy 2.0 safe
...
def main():
    ...
    steering_cache_dir = Path("steering")  # where vectors + rms cache live
    ...
    logger.info(f"Eval: regex expressions (not exact string match). Save gens={bool(args.save_gens)}")
    ...
    logger.info(f"Model loaded: L={L} D={D} input_device={input_device}")
    ...
    # RMS normalization (optional but recommended)
    rms_by_layer = None
    if args.alpha_unit == "rms":
        ...
        if len(rms_by_layer) < L:
            logger.info(f"RMS computed for Luse={len(rms_by_layer)} layers (file had fewer layers).")

    # Load instances for this mode/split
    ...
    current_steer = {
        ...
        "alpha_eff": 0.0,     # float scalar actually applied
    ...
    # Register hooks for all layers (they only activate when current_steer["layer"] matches)
    hooks = []
    ...
    tag = (f"{args.template}_{args.split}_{args.steer_space}_{args.steer_mode}"
           f"_unit{args.alpha_unit}_lto{int(args.last_token_only)}_pso{int(args.prompt_step_only)}"
    ...
    for layer in sweep_layers:
        ...
        for alpha in args.alphas:
            ...
            for inst in instances:
                ...
                if args.alpha_unit == "rms":
                    if rms_by_layer is None or layer >= len(rms_by_layer):
                        raise RuntimeError(f"RMS missing for layer {layer}. rms_by_layer_len={len(rms_by_layer) if rms_by_layer is not None else None}")
                ...
                if gens_f:
                    gens_f.write(json.dumps({
                        ...
                        "alpha_eff_scale": float(rms_by_layer[layer]) if (args.alpha_unit == "rms" and rms_by_layer is not None and layer < len(rms_by_layer)) else 1.0,
