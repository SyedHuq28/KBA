"""
2_extract_allayer_with_sad.py

Merged single-pass extractor:
- Extract last-token hidden states H for every transformer layer
- Extract last-token MLP outputs M for every transformer layer
- Extract SAD v2 features from last-query attention row:
    (a) features_mean      : 7 features after averaging attention across layers
    (b) features_by_layer  : [n_layers, 7] features per layer

Saves EXACTLY the same outputs/paths as your previous scripts:

1) First script outputs (unchanged):
   activations/H_{template}_{split}.npy
   activations/M_{template}_{split}.npy
   activations/y_{template}_{split}.npy
   activations/meta_{template}_{split}.json

2) Second script output (same default filename/location):
   sad_features_v2_alllayers.json

INDEXING CONVENTION (critical — never change):
  H[i, layer_idx, :] = out.hidden_states[layer_idx + 1][0, -1, :]
                      = output of layers[layer_idx], last token
  In routing/hook code: use layer_idx directly — NO +1 needed anywhere else.

Notes:
- Uses ONE forward pass per example: output_hidden_states=True + output_attentions=True
- Qwen/Qwen3-safe default: torch_dtype=torch.bfloat16
- Hooks do NOT modify forward outputs; they only store detached CPU float32 copies
- We preserve the first script's prompt exactly via render_prompt(...)
- For SAD doc token spans, we recover document token spans by matching tokenized docs
  inside the tokenized full prompt in order. This works if docs appear verbatim in the
  rendered prompt. If your prompts.py reformats/truncates/paraphrases docs, then you
  should instead make prompts.py return explicit boundaries.

Usage examples:
  python 2_extract_allayer_with_sad.py --template P0 --split train
  python 2_extract_allayer_with_sad.py --template P0 --split val
  python 2_extract_allayer_with_sad.py --template P0 --split test

  # prompt-dependence templates:
  for tmpl in P1 P2 P3 P4 P5; do
    python 2_extract_allayer_with_sad.py --template $tmpl --split val
  done
"""
