"""
2_extract_allayer.py
Extract last-token hidden states (H) and MLP outputs (M) at every
transformer layer, for every instance in a given split/template.
...
Qwen3-safe fixes:
  - Use torch_dtype=torch.bfloat16 (Qwen3 weights are bf16; fp16 can mismatch)
  - Forward hooks must NOT change the dtype of tensors used by the forward pass.
    We only store detached CPU float32 copies.
  - Use model-agnostic get_layers() + get_mlp() so model swaps are safer.
  - Store outputs as float16 by default (disk-friendly); can switch to float32.

Usage:
  python 2_extract_allayer.py --template P0 --split train
  python 2_extract_allayer.py --template P0 --split val
  python 2_extract_allayer.py --template P0 --split test
  # Prompt-dependence templates (val/test recommended):
  for tmpl in P1 P2 P3 P4 P5; do
    python 2_extract_allayer.py --template $tmpl --split val
  done
"""
import argparse
import gc
