#!/usr/bin/env python3
"""
9_causal_patching.py
Causal patching for the ROUTER representation.

Goal:
  Replace the target instance's last-token hidden state at router layer(s)
  with the source instance's hidden state (same layer(s)), then see if the
  router prediction flips toward the source mode.

Why:
  Probes show predictiveness (correlation). Patching tests causal influence
  of the representation used by the router.

Usage (dev router on VAL):
  python 9_causal_patching.py \
    --model Qwen/Qwen3-4B-Instruct-2507 \
    --router_pkl results/router_hidden_dev.pkl \
    --split val --template P0 \
    --n_pairs 30 \
    --out_file results/causal_patching_val.json

Usage (final router on TEST):
  python 9_causal_patching.py \
    --model Qwen/Qwen3-4B-Instruct-2507 \
    --router_pkl results/router_hidden_final.pkl \
    --split test --template P0 \
    --n_pairs 60 \
    --out_file results/causal_patching_test.json
"""
