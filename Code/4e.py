#!/usr/bin/env python3
"""
4b.py — Steering sweep with UNIFIED triage instruction (Option A).

Core idea:
- Prompt DOES NOT change by mode (same instruction always).
- Only the steering direction changes (refuse vs conflict vector).

This improves generalization & makes comparisons fair.

Outputs:
- Summary: steering/steering_sweep_<tag>.json
- Generations: steering/steering_gens_<tag>.jsonl.gz (gen_text field)

Evaluation:
- Regex mode classifier (Answer/Refuse/Conflict) + FAR
- If you use unified exact strings, you can also rely on exact match later.

NumPy 2.0 safe:
- RMS computed chunked; no np.array(copy=False) usage.
"""
