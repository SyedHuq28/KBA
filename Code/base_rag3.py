#!/usr/bin/env python3
"""
cad_rag.py  —  Context-Aware Decoding (CAD) for RAG evaluation.

CAD formula (Shi et al., 2023):
    logits_CAD(x | ctx, q) = logits(x | ctx, q) - alpha * logits(x | q)

Three outputs per instance:
    cad_output   — CAD-decoded (our method)
    ctx_output   — standard RAG greedy (context-only baseline)
    noctx_output — no-context greedy (Group 1 / origin without context)

Key fixes vs previous version:
    1. Integer labels (0,1,2) are mapped to ("answer","refuse","conflict")
    2. Token-F1 soft match works correctly for answer instances
    3. Files always saved (smoke test and full run)

Usage:
    python cad_rag.py --model qwen3-4b --data instances_test.jsonl --smoke_test --output results/smoke.jsonl
    python cad_rag.py --model qwen3-4b --data instances_test.jsonl --alpha 0.5 --output results/cad.jsonl
"""
