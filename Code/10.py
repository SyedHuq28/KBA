"""
10_sad_reextract_all_layers.py
SAD v2 feature extraction with robust token-boundaries + last-query attention row.

Upgrades vs your snippet:
- Works on ALL layers by default (or any subset via CLI).
- Saves BOTH:
  (a) features_mean: 7 features computed after averaging attention across layers (your original approach)
  (b) features_by_layer: [n_layers, 7] features computed per-layer (so you can train on "all layers").

Output JSON: sad_features_v2_alllayers.json

Assumptions about instances_*.jsonl:
Each row has at least: id, split, true_mode (or label), question, docs (list), retriever_scores (list)
"""
