#!/usr/bin/env python3
"""
6_router_and_system.py
Train a calibrated multinomial logistic router on the best hidden-state layer (or a band),
and save router+scaler+metadata as a pickle.

Key points:
  - Avoid val leakage by default: use --train_mode train to train on train and calibrate on val.
  - For final reporting: use --train_mode trainval (train+val) and internal CV calibration.
  - Uses best layer from results/layer_probe_hidden.json unless overridden.
  - device_map-safe: this script doesn't run the model; it uses cached activations.

Outputs:
  results/router_hidden_dev.pkl     (train_mode=train)
  results/router_hidden_final.pkl   (train_mode=trainval)
"""
